...
**Last updated**: 2026-05-09 (Phase 6-B + 21-B + 9-B Tier B + 10-B Tier B close + earlier 12-B / 13-B Tier B / Memory / Analytics — visible-factory sprint complete; sidebar 11/11 real, zero static mocks)
...
| B-tier deferred (production wiring) | 13 subphases pending — 6-B + 12-B + 21-B closed (full); 9-B + 10-B + 13-B partial (Tier B closed, remainder needs creds/sandbox) |
| Decisions logged | D1–D210 (added D209-D210 at 21-B + 6-B close) |
...
| **6** | Artifact-First State | ✅ **closed (6-A + 6-B)** | `cognitive-engine/artifacts/`: types, store, sha256, monotonic IDs. **6-B**: artifact dual-write hook in `RouterChatModel.invoke()` (chokepoint covers all LLM calls in one patch); `setProjectDir()` populates `AGENT_PROJECT_DIR` + `AGENT_RUN_ID` env vars so all 3 graphs auto-activate when `USE_ARTIFACT_STORE` flips on; fail-open (artifact-write errors caught + warned + swallowed). E2E verified — artifacts → `listRunIds` → `collectRun` → Replay UI populates. D210 captured. | — | — |
...
| **9** | Verification Queue | 🟡 partial (9-A + 9-B Tier B read UI) | `cognitive-engine/verify-integration/`: BuildBundle/FeedbackPayload contracts, mock client, feedback receiver writes user_note observations + plans fix-cycle routes. **Admin → ✅ Verify tab**: enabled flag / client kind / VERIFY_URL / REVIEW_DECISIONS pills / recent published bundles list. | 9-B remainder: real HTTPS to `verify-stg.agentryx.dev`, webhook endpoint, real fix-cycle agent invocation | Verify multi-app mode + auth_token |
| **10** | Courier (External Comms) | 🟡 partial (10-A + 10-B Tier B read UI) | `cognitive-engine/courier/`: 8 event types × 6 channels × 3 backends (fake/http/null); routing config with severity filter + `$project_id` substitution. **Admin → 📡 Courier tab**: 4-stat strip + EVENT_TYPES list + CHANNELS pills + SEVERITIES pills + live routing config JSON viewer. | 10-B remainder: Hermes container in gateway mode, channel bindings, producer wiring, in-memory recent_events ring buffer | User creds (Slack/GitHub/email) + Hermes deploy |
...
| **21** | Master Architect | ✅ **closed (21-A + 21-A.1 + 21-B)** | **21-A**: substrate — types + KB + scheduler + researcher + proposer + applier + portal API; standing_orders schema; D190-D199. **21-A.1**: Platform Evolution Roadmap (3 cadences daily/weekly/monthly, embedded daemon, IST-aware) + Founder R&D Brief + Seven (12th named agent, first SOUL.md) + `tool_evaluation` finding kind + Hermes Lab→`testing` + Master Architect 3-tab UI; D200-D204. **21-B** (this session): `cognitive-engine/architect/dispatchers/llm.js` — `createLLMDispatcher({dispatcher:'sonnet'\|'opus'})` drop-in replacement for stub; routes through llm-router `complete()` (research/architect tasks); strict JSON output contract; cost capture + budget caps via the router; opt-in per cadence/brief (default stays stub); D209. | Phase 22 only (action-boundary enforcement at v2→v3 — sandbox runtimes, signed manifests) | — |
