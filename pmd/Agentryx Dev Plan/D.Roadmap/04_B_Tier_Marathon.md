...
- **19-A** ✅ in main since 2026-04-24 — customer-portal substrate, 138 smoke assertions. **19-B**: project_intake queue handler (#66, D224) + HTTP surface (#67, D225, 6 endpoints under `/api/customer-portal/*` with bearer auth + auto-enqueue) + idempotency hotfix (#68, D226 — caught by live test) + **pre_dev → delivered back-feed wrapper (#70, D227, 78-assertion smoke)** + **SLA breach scanner daemon (#71, D228 + #72 D229 hotfix, 86-assertion smoke; tick every 5 min; opt-out via `SLA_SCANNER_DISABLED=true`)** + **portal notifier with scanner-first wiring (D230, 51-assertion notifier smoke + 26 new scanner-integration assertions = 163 across both modules; 6 new `customer.*` Courier event types + 6 routing rules; default fake backend; opt-out via `NOTIFIER_DISABLED=true`)** all shipped 2026-05-18. **Live end-to-end verified through pre_dev intake**: registered customer via HTTP → submitted project → project_intake handler ran clean on attempt 1 → downstream pre_dev job enqueued with customer-prefixed project_id. Wrapper is wired into bootQueueWorker; full-loop verification (submission → pre_dev → delivered) verified via real-portal integration script. Scanner emits `sla_breached` timeline events idempotently AND now dispatches `customer.sla_breached` Courier events on every fresh breach. **What remains**: React customer dashboard + other 3 notifier-source wirings (D231-D233 — HTTP /submit + intake + back-feed) + per-customer notification prefs (19-C) + admin auth (Phase 22).
...
- **Portal notifier — all 6 customer.* sources wired** (opt-out via `NOTIFIER_DISABLED=true`; backend selectable via `COURIER_BACKEND` env, default fake = in-memory history). The full notification taxonomy fires end-to-end on real LLM-eligible code paths: HTTP /submit → `onSubmitted` (D231) · project_intake handler → `onAccepted` (D232) · back-feed wrapper → `onDelivered` (D232) · HTTP /cancel → `onCancelled` (D231) · admin /reject → `onRejected` (D233) · SLA scanner → `onSlaBreached` (D230). All visible in the Dev-Hub Notifications panel + (for admin actions) a Reject button on the Customer Portal tab. Per-customer prefs (real Slack/email) is the remaining 19-C ship.
