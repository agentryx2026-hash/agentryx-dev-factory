...
- **Portal notifier wired into scanner** (opt-out via `NOTIFIER_DISABLED=true`; backend selectable via `COURIER_BACKEND` env, default fake = in-memory history) — every fresh SLA breach dispatches a `customer.sla_breached` Courier event in addition to the timeline write. 6 `customer.*` event types registered; routing all → stdout for v0.0.1 (founder log); per-customer prefs is 19-C work
